...
contract('DALAuction', function ([owner, purchaser, beneficiary]) {
    ...
    it('should be token owner', async function () {
        assert.equal(owner, await this.vault.wallet())
    })
    ...
    it('should reject bids after end', async function () {
        ...
        const amount = {value: ether(1), from: beneficiary}
        await this.auction.sendTransaction(amount).should.be.rejectedWith(EVMThrow)
        await this.auction.placeBid(amount).should.be.rejectedWith(EVMThrow)
    })

    it('should reject bids after auction finalization', async function () {
        await this.auction.freezeBook();
        await this.auction.finalizeAuction().should.be.fulfilled;
        (await this.auction.phase()).should.be.bignumber.equal(AuctionPhase.Finalized);
        const amount = {value: ether(1), from: beneficiary}
        await this.auction.sendTransaction(amount).should.be.rejectedWith(EVMThrow)
